...
namespace ADMIN
{
    ...
    {
        frm_main menu;
        public frm_horarios(frm_main anterior)
        {
            InitializeComponent();
            menu = anterior;
            actualizar();
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_HORARIO FROM HORARIO WHERE ID_HORARIO = '{0}'", textBox1.Text);
                    ...
                    if (!reader.Read() || (textBox3.Text == textBox1.Text))
                    {
                        ...
                        sql = string.Format("UPDATE HORARIO SET ID_HORARIO = '{0}' WHERE ID_HORARIO = '{1}';", textBox1.Text, textBox3.Text);
        ...
        private void Btn_formatoAceptar_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_HORARIO FROM HORARIO WHERE ID_HORARIO = '{0}'", textBox2.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (!reader.Read())
                    {
                        conexion.Close();
                        conexion = TaquillaDB.getDB();
                        sql = string.Format("INSERT INTO HORARIO VALUES('{0}');", textBox2.Text);
                        cmd = new OdbcCommand(sql, conexion);
                        cmd.ExecuteNonQuery();
                        textBox2.Text = "";
                        textBox2.Focus();
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM HORARIO WHERE ID_HORARIO = '{0}'", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
