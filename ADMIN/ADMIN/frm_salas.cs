...
namespace ADMIN
{
    ...
    {
        frm_main menu;
        public frm_salas(frm_main anterior)
        {
            InitializeComponent();
            menu = anterior;
            actualizar();
        ...
        private void cine()
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_CINE FROM CINE;");
        ...
        private void formato()
        {
            ...
            try
            {
                string sql = string.Format("SELECT ID_FORMATO FROM FORMATO;");
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (textBox4.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM SALA WHERE ID_SALA = '{0}'", textBox4.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        private void Btn_formatoAceptar_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "" && textBox5.Text != "" && comboBox1.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("INSERT INTO SALA VALUES(NULL, {0}, {1}, {2}, '{3}', {4});", textBox2.Text, textBox5.Text, comboBox1.Text, comboBox3.Text, textBox7.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    cmd.ExecuteNonQuery();
                    textBox2.Text = "";
                    textBox5.Text = "";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM SALA WHERE ID_SALA = '{0}'", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "" && textBox6.Text != "" && textBox6.Text != "" && comboBox4.Text != "" && comboBox2.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("UPDATE SALA SET FILAS_SALA = {0}, COLUMNAS_SALA = {1}, ID_CINE = {2}, ID_FORMATO = '{3}', NUMERO_SALA = '{4}' WHERE ID_SALA = {5}", textBox6.Text, textBox1.Text, comboBox4.Text, comboBox2.Text, textBox8.Text, textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    cmd.ExecuteNonQuery();
