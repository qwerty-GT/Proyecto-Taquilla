...
namespace ADMIN
{
    ...
    {
        frm_main menu;
        public frm_peliculas(frm_main anterior, bool[] privilegios)
        {
            InitializeComponent();
            menu = anterior;
            clasif();
        ...
        private void Btn_formatoAceptar_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "" && textBox5.Text != "" && textBox7.Text != "" && comboBox1.Text != "" && comboBox3.Text != "" && comboBox4.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("INSERT INTO PELICULA VALUES (NULL, '{0}', '{1}', '{2}', (SELECT ID_CLASIFICACION FROM CLASIFICACION WHERE DESCRIPCION_CLASIFICACION = '{3}'), (SELECT ID_GENERO FROM GENERO WHERE DESCRIPCION_GENERO = '{4}'), '{5}'); ", textBox2.Text, comboBox1.Text, textBox5.Text, comboBox3.Text, comboBox4.Text, textBox7.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    cmd.ExecuteNonQuery();
                    textBox2.Text = "";
                    textBox5.Text = "";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT P.ID_PELICULA, P.NOMBRE_PELICULA, P.ESTADO_PELICULA, P.SINOPSIS_PELICULA, C.DESCRIPCION_CLASIFICACION, G.DESCRIPCION_GENERO, P.DURACION_PELICULA FROM PELICULA P, CLASIFICACION C, GENERO G WHERE P.ID_CLASIFICACION = C.ID_CLASIFICACION AND P.ID_GENERO = G.ID_GENERO AND ID_PELICULA = {0}", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
