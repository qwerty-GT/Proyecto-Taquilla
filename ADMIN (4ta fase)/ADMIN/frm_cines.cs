...
namespace ADMIN
{
    ...
    {
        frm_menu menu;
        public frm_cines(frm_menu anterior, bool[] privilegios)
        {
            InitializeComponent();
            menu = anterior;
            deptos();
        ...
        private void actualizar()
        {
            ...
            try
            {
                string sql = string.Format("SELECT C.ID_CINE, C.NOMBRE_CINE, C.DIRECCION_CINE, D.NOMBRE_DEPARTAMENTO FROM CINE C, DEPARTAMENTO D WHERE D.ID_DEPARTAMENTO = C.ID_DEPARTAMENTO;");
                OdbcCommand cmd = new OdbcCommand(sql, conexion);
                OdbcDataReader reader = cmd.ExecuteReader();
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT C.ID_CINE, C.NOMBRE_CINE, C.DIRECCION_CINE, D.NOMBRE_DEPARTAMENTO FROM CINE C, DEPARTAMENTO D WHERE D.ID_DEPARTAMENTO = C.ID_DEPARTAMENTO AND C.ID_CINE = '{0}'", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        private void frm_cines_Load(object sender, EventArgs e)
        {
