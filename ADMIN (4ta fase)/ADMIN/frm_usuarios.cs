...
namespace ADMIN
{
    ...
    {
        frm_menu menu;
        public frm_usuarios(frm_menu anterior, bool[] privilegios)
        {
            InitializeComponent();
            menu = anterior;
            actualizar();
        ...
        private void Btn_gestionAceptar_Click(object sender, EventArgs e)
        {
            if (Txt_gestionUsuario.Text != "" && Txt_gestionPass.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT USU_USUARIO FROM USUARIO WHERE USU_USUARIO = UPPER('{0}')", Txt_gestionUsuario.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (!reader.Read())
                    {
                        ...
                        sql = string.Format("INSERT INTO USUARIO VALUES(UPPER('{0}'),MD5('{1}'),{2},{3},{4},{5});", Txt_gestionUsuario.Text, Txt_gestionPass.Text, Convert.ToInt32(checkBox2.Checked), Convert.ToInt32(checkBox3.Checked), Convert.ToInt32(checkBox4.Checked), Convert.ToInt32(checkBox5.Checked));
        ...
        private void button1_Click_2(object sender, EventArgs e)
        {
            if (textBox4.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("DELETE FROM USUARIO WHERE USU_USUARIO = UPPER('{0}')", textBox4.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (textBox4.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM USUARIO WHERE USU_USUARIO = UPPER('{0}')", textBox4.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        private void Txt_gestionUsuario_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == ' ')
            {
                e.Handled = true;
        ...
        private void Txt_gestionPass_TextChanged(object sender, EventArgs e)
        {
        }
