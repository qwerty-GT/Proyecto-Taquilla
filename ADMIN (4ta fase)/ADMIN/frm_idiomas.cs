...
namespace ADMIN
{
    ...
    {
        frm_menu menu;
        public frm_idiomas(frm_menu anterior, bool[] privilegios)
        {
            InitializeComponent();
            menu = anterior;
            actualizar();
        ...
        private void Btn_formatoAceptar_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_IDIOMA FROM IDIOMA WHERE DESCRIPCION_IDIOMA = '{0}'", textBox2.Text);
                    ...
                    if (!reader.Read())
                    {
                        ...
                        sql = string.Format("INSERT INTO IDIOMA VALUES(NULL, '{0}');", textBox2.Text);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("ESCRIBA UN ID IDIOMA!", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox4.Focus();
            }
        }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT DESCRIPCION_IDIOMA FROM IDIOMA WHERE ID_IDIOMA = {0}", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_IDIOMA FROM IDIOMA WHERE DESCRIPCION_IDIOMA = '{0}'", textBox1.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (!reader.Read() || (textBox3.Text == reader.GetString(0)))
                    {
                        conexion.Close();
                        conexion = TaquillaDB.getDB();
                        sql = string.Format("UPDATE IDIOMA SET DESCRIPCION_IDIOMA = '{0}' WHERE ID_IDIOMA = {1};", textBox1.Text, textBox3.Text);
                        cmd = new OdbcCommand(sql, conexion);
                        cmd.ExecuteNonQuery();
