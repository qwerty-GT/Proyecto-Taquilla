...
namespace ADMIN
{
    public partial class frm_reporte_peliculas_taquilleras : Form
    {
        ...
        public frm_reporte_peliculas_taquilleras(frm_reportes anterior, bool[] privilegios)
        {
            InitializeComponent();
            reporte = anterior;
        }
        ...
        public void CargarGrid1()//carga la informacion del dg1
        {
            ...
            try
            {
                string sql = string.Format("SELECT P.NOMBRE_PELICULA,COUNT(*) AS RESERVACIONES FROM PELICULA AS P, FUNCION AS F, RESERVACION AS R,FACTURA AS Q WHERE P.ID_PELICULA=F.ID_PELICULA AND R.ID_FUNCION=F.ID_FUNCION AND R.ID_FACTURA=Q.ID_FACTURA AND Q.FECHA_FACTURA BETWEEN '{0}' AND '{1}' GROUP BY R.ID_FUNCION;", dateTimePicker2.Text,dateTimePicker1.Text);
        ...
        private void To_pdf()
        {
            ...
            if (filename.Trim() != "")
            {
                ...
                Chunk chunk = new Chunk("REPORTE DE PELICULAS MAS TAQUILLERAS CINEMA LICAS", FontFactory.GetFont("ARIAL", 20, iTextSharp.text.Font.BOLD));
        ...
        private void frm_reporte_peliculas_taquilleras_FormClosing(object sender, FormClosingEventArgs e)
        {
            reporte.Show();
        }
    }
