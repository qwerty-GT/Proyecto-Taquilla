    ...
    public partial class frm_generos : Form
    {
        ...
        private void Btn_formatoAceptar_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_GENERO FROM GENERO WHERE ID_GENERO = '{0}'", textBox2.Text);
                    ...
                    if (!reader.Read())
                    {
                        ...
                        sql = string.Format("INSERT INTO GENERO VALUES('{0}');", textBox2.Text);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (textBox1.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT ID_GENERO FROM GENERO WHERE ID_GENERO = '{0}'", textBox1.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (!reader.Read() || (textBox3.Text == textBox1.Text))
                    {
                        conexion.Close();
                        conexion = TaquillaDB.getDB();
                        sql = string.Format("UPDATE GENERO SET ID_GENERO = '{0}' WHERE ID_GENERO = '{1}';", textBox1.Text, textBox3.Text);
                        cmd = new OdbcCommand(sql, conexion);
                        cmd.ExecuteNonQuery();
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                ...
                try
                {
                    string sql = string.Format("SELECT * FROM GENERO WHERE ID_GENERO = '{0}'", textBox3.Text);
                    OdbcCommand cmd = new OdbcCommand(sql, conexion);
                    OdbcDataReader reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
